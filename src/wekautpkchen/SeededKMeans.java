...
public class SeededKMeans extends Clusterer implements OptionHandler, SemiSupClusterer, ActiveLearningClusterer {
    ...
    /**
     * Is the objective function increasing or decreasing? Depends on type of
     * metric used: for similarity-based metric, increasing, for distance-based
     * - decreasing
     */
    ...
    /**
     * Points that are to be skipped in the clustering process because they are
     * collapsed to zero
     */
    ...
    /**
     * m_FastMode = true => fast computation of meanOrMode in centroid
     * calculation, useful for high-D data sets m_FastMode = false => usual
     * computation of meanOrMode in centroid calculation
     */
    ...
    /**
     * We always want to implement SemiSupClusterer from a class extending
     * Clusterer. We want to be able to return the underlying parent class.
     *
    ...
    /**
     * Clusters unlabeledData and labeledData (with labels removed), using
     * labeledData as seeds
     *
     ...
     * @param startingIndexOfTest from where test data starts in unlabeledData,
     * useful if clustering is transductive
     * @exception Exception if something goes wrong.
     */
    ...
    /**
     * Generates a clusterer. Instances in data have to be either all sparse or
     * all non-sparse
     *
    ...
    /**
     * Finds point which is nearest to center. This point should not be a test
     * point and should not belong to visitedPoints
     */
    ...
    /**
     * This function divides every attribute value in an instance by the
     * instance weight -- useful to find the mean of a cluster in Euclidean
     * space
     *
    ...
    /**
     * Checks if instance has to be normalized and classifies the instance using
    ...
    /**
     * Set the minimum value of the objective function difference required for
     * convergence
     *
     * @param objFunConvergenceDifference the minimum value of the objective
     * function difference required for convergence
     */
    ...
    /**
     * Get the minimum value of the objective function difference required for
     * convergence
     *
     * @returns the minimum value of the objective function difference required
     * for convergence
     */
    ...
    /**
     * Set the seeding method. Values other than SEEDING_CONSTRAINED, or
     * SEEDING_SEEDED will be ignored
     *
    ...
    /**
     * Set the KMeans algorithm. Values other than ALGORITHM_SIMPLE or
     * ALGORITHM_SPHERICAL will be ignored
     *
    ...
    /**
     * Get the KMeans algorithm type. Will be one of ALGORITHM_SIMPLE or
     * ALGORITHM_SPHERICAL
     *
    ...
    /**
     * Read the seeds from a hastable, where every key is an instance and every
     * value is: the cluster assignment of that instance seedVector vector
     * containing seeds
     */
     ...
     */
    public void writeClusterResults(String filename) throws Exception {
        System.out.println("now writing clustering results to:" + filename);
            ...
            if (cluster == null) {
             //   System.out.println("Cluster " + i + " is null");
              ...
              //  System.out.println("Cluster " + i + " consists of " + cluster.size() + " elements");
                for (Object cluster1 : cluster) {
                    int idx = ((Integer) cluster1);
                  //  System.out.println("\t\t" + idx);
                    outputWriter.write(i + "\t"+idx);
                    outputWriter.newLine();
                }
            }
        }
        System.out.println("finish writing clustering results to:" + filename);
        outputWriter.flush();
        outputWriter.close();
    }
    ...
    /**
     * Computes the final clusters from the cluster assignments, for external
     * access
     *
     * @exception Exception if clusters could not be computed successfully
     */
    ...
    /**
     * Fast version of meanOrMode - streamlined from Instances.meanOrMode for
     * efficiency Does not check for missing attributes, assumes numeric
     * attributes, assumes Sparse instances
     */
    ...
    public static void readSeedIndices(String seedfile) throws FileNotFoundException {
        String tmp;
        String[] cells;
...
//"/u/ml/software/weka-latest/data/iris.arff"
                // set up the data
